...
public interface ReportRepository extends JpaRepository<Report, Long> {
    ...
    @Query("select r from Report r where r.chatUser.id = ?1")
    Optional<List<Report>> findByUserId(Long userId);

    @Query(value = "select max (r) from Report r where r.chatUser.id=?1")
    Optional<Report> findLastReportByUserId(Long userId);

    @Query("select max (r.sentDate) from Report r where r.chatUser.id = ?1")
